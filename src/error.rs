...
pub enum Error {
    ...
    OpenFile(alloc::string::String, firefly_hal::FSError),
    ReadFile(alloc::string::String, firefly_hal::FSError),
    NoLauncher,
...
impl fmt::Display for HostError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            HostError::DataFileInNet => write!(f, "cannot read data files in multiplayer"),
            HostError::OomPointer => write!(f, "buffer points out of memory"),
            HostError::FileRead(e) => write!(f, "cannot read file: {e}"),
            HostError::FileWrite => write!(f, "cannot write file"),
            ...
            HostError::InvalidWidth => write!(f, "the image has invalid width"),
